\chapter{AirLogic}
  ...
  \section{Introduction}
    ...
    % Change this paragraph in the final version to make it sound to "our" vision of PaP.
  ...
  \section{\al Operating Principle}
    ...
    \cref{fig:teaser}, A; later sections describe these components in
    greater detail.
    ...
    illustrated in \cref{fig:logic-widgets}, B), a single jet proceeds
    ...
    \begin{figure}[H]
      \includegraphics[width=\minof{\columnwidth}{0.5\textwidth}]
        {example-image-duck}
      \caption{Operating principle}
      \label{fig:app-slider}
      \Description{An awesome duck.}
    \end{figure}
  ...
  \section{Related Work}
    ...
    fabrication of interactive objects, fluidics, and analog computing.
      ...
      \label{sec:fluerics}
        ...
        created~\cite{Glaettli:1964}, as shown in \cref{fig:logic-widgets}.
    ...
    \subsection{Input widgets}
      ...
      \begin{figure}[htb]
          \centering \includegraphics[width=0.8\columnwidth]
            {figures/InputOutputWidgets_v5.pdf}
      ...
      \begin{enumerate}
        ...
        \item \emph{Push Button}. 
          ...
          push button (\cref{fig:input-widgets}, B). The cantilever spring
      ...
      \subsection{Logic Widgets}
        ...
        \cref{sec:fluerics}). Our use of flueric structures to represent
        ...
        \begin{enumerate}
          \item \texttt{AND}. 
            ...
            (\cref{fig:logic-widgets}-A,1) inspired by classic works in
            ...
            present (\cref{fig:logic-widgets}-A,2), the flow is directed to
            the respective vent channel. If both inputs are present,
            however, their corresponding jets collide, redirecting the flow
            to the output channel (\cref{fig:logic-widgets}-A,3).

          \item \texttt{OR}. 
            Our \texttt{OR} logic gate widget
            (\cref{fig:logic-widgets}-B,1) is also based on classic works
            ...
            (\cref{fig:logic-widgets}-B,2), the flow is directed to the
          ...
          \item \texttt{NOT}. 
            ...
            it (\cref{fig:logic-widgets}-D,1). In this configuration the power
            channel provides a constant flow of air, which is directed at
            the output channel (\cref{fig:logic-widgets}-D,2). If there's flow
            incoming from the input channel, this jet collides with the
            power jet, causing the flow to be redirected to a vent
            (\cref{fig:logic-widgets}-D,3).
        \end{enumerate}
        
        \begin{figure}[H]
          \includegraphics[width=\minof{\columnwidth}{0.7\textwidth}]
            {figures/LogicGates_v2.pdf}
      ...
      \subsection{Output Widgets}
        ...
        \begin{enumerate}
          ...
          \item \emph{Oscillating actuator}. 
            ...
            (\cref{fig:output-widgets}-C), comprised of a lever that is
          ...
          \item \emph{Vibration motor}. 
            ...
            (\cref{fig:output-widgets}-D). This widget operates similarly
        ...
        \begin{figure}[H]
          \includegraphics[width=\minof{0.8\columnwidth}{0.5\textwidth}]
            {figures/Widget_Output.pdf}
  ...
  \section{Designing \al Objects}
    ...
    for Autodesk Fusion 360, shown in \cref{fig:design-environment}. Our
    ...
    While prototyping with encapsulated widgets is a heavily manual process for the designer and involves making many connections, fabricating and assembling an \al-powered 3D model with internal widgets requires little intervention. The majority
    of the object is printed as a single structure, involving only the
    assembly of moving parts (e.g., button, slider), and aesthetic covers.

    \begin{figure}[H]
      \includegraphics[width=\minof{\columnwidth}{0.5\textwidth}]
        {figures/Design Environment.png}
      ...
      \Description{The widget library (right) and a Dial widget dropped by
        using the interface. In the interface, users can choose widgets and
        insert them into their design.}
    \end{figure}
  ...
  \section{Implementation}
    ...
    by embedding dissolvable, breakable or \red{other support type here}
  ...
  \section{Validation}
    ...
    \subsection{Technical Evaluation}
      ...
      \subsubsection{Results}
        ...
        with a constant air source with pressures from \red{999}
        kilopascals (kPa), to \red{999} kPa. In contrast, our logic widgets
        presented an operating range of \red{000} to \red{000} kPa, while
        our output widgets operate best when actuated with pressured from
        \red{111} to \red{111} kPa. \cref{fig:operating-ranges} presents
        the results of our explorations.

        \begin{figure}[H]
          \includegraphics[width=\minof{\columnwidth}{0.5\textwidth}]
            {example-image-duck}
          \caption{Operating ranges from our widgets}
          \label{fig:operating-ranges}
          \Description{An awesome duck.}
        \end{figure}

        In addition, we found that, in average, our logic widgets lose
        \red{111} kPa of pressure when processing their respective
        operations, while our input widgets designs present a pressure loss
        of \red{444} kPa. We did not evaluate the pressure losses of our
        output widgets, as they are intended to be the last element in our
        \red{circuits}. A detailed view of our results can be found in
        \cref{fig:pressure-losses}.
    ...
    \subsection{Example Applications}
      ...
      \subsubsection{Handheld pin display}
        \label{sec:handheld}
        To illustrate how \al objects can be powered using different
        sources of air, we constructed a handheld pin display. Instead of
        using a compressor, users can power \al using their lungs. In this
        example, depending on what combination of buttons are pressed, a
        different pin is raised.
        
        \begin{figure}[H]
          \includegraphics[width=\minof{\columnwidth}{0.5\textwidth}]
            {example-image-duck}
          \caption{Handheld pin display}
          \label{fig:app-slider}
          \Description{An awesome duck.}
        \end{figure}
  ...
  \section{Discussion \& Limitations}
    \subsection{Scalability}
      Our characterization efforts for our widgets uncovered the
      operating barometric pressure range, and intrinsic pressure losses in
      our designs. Using these values as reference, given a constant air
      source of \red{999} kPa~\footnote{For reference, human lungs can
      pressurize up to \red{12.4} kPa.}, we can connect up to \red{9}
      widgets in series to create an \al object.

      \subsubsection{Chaining logic widgets}
        While our applications illustrate the capacity of using multiple
        logic widgets in an \al object (see \cref{sec:pitch} and
        \cref{sec:puzzle}), this functionality is limited in our current
        implementation. During exploratory tests we found that our logic
        widget designs could be connected three different ways: in
        parallel, balanced chained \texttt{AND}, and chained \texttt{OR}. We are not able to support unbalanced chained \texttt{AND}, and combinations.
        ...
        based on passive fluidic logic devices, where the \red{some
        description of passive devices}. Using active designs, which
        ...
        that of the power air jet. This can allow \red{something}. Second,
      ...
      \subsubsection{Exploring different fabrication methods.} 
        ...
        Additionally, we wish to explore subtractive fabrication methods
        for constructing our widgets. We believe that because of its high
        precision and clean cuts, laser cutting equipment presents a
        promising alternative to fabricate \al objects. While \al objects
        constructed using laser cutting equipment would require significantly
        more manual assembly than our current implementation and would likely be limited to 2D, we envision a
        further iteration of our design environment to provide stencils and
        assembly guides for \al designs fabricated on such machinery.
      ...
      \subsubsection{Other air sources}
        ...
        objects using their lungs (see \cref{sec:handheld}), and designers
