\chapter{\bh} \label{ch:blowhole}
  ...
  While \at is a successful technique for constructing tangible devices that can
  respond to user's touch, it had some serious limitations. The most prominent
  of these limitations was the need of a constant, pressurized source of air,
  like an air compressor. Additionally, having to connect the device using two
  points of entry (i.e., one to the air source, another to the sensing
  apparatus) meant that the designer had to perform a few post-print activities,
  albeit quick and easy ones.

  Inspired by these limitations, and the ubiquity of microphones in our daily
  lives, I developed \bh. This next chapter, which is almost identical to the
  paper I published by the same name~\cite{Tejada:2018}, and introduces a
  technique to fabricate tangible devices that can identify user's interactions
  by acoustically tag locations on the object. In this chapter, I introduce
  \bh's theory of operation, and characterization and evaluation procedures I
  carried out to assess its performance.

  \bh improves on \at in two main ways. First, as it employs acoustic sensing
  rather than pneumatic sensing, users can provide the air for it to operate by
  gently blowing on the object's openings. Second, thanks to the ubiquity of
  microphones, \bh-enabled objects can employ already existing microphones to
  identify user's interactions. Additionally, \bh uses a mathematical model to
  identify blowing locations based on the frequency emitted with gently blown.
  This means that an user can use a \bh-enabled device immediately after
  ...
  \begin{figure}
    \centering
    \subfloat[]{\label{fig:elephant}\includegraphics[height=.21\textwidth]{print-and-play/blowhole/elephant}}
    \subfloat[]{\label{fig:bar_rand}\includegraphics[height=.21\textwidth]{print-and-play/blowhole/bar_rand1}}
    \subfloat[]{\label{fig:whales}\includegraphics[height=.21\textwidth]{print-and-play/blowhole/whales}}
    \subfloat[]{\label{fig:cell}\includegraphics[height=.21\textwidth]{print-and-play/blowhole/cell}}
